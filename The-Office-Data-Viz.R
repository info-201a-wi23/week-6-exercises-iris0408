...
highest_rating_rows <- office_df %>% filter(Rating ==  max(Rating, na.rm = T))
...
lowest_rating_rows <- office_df %>% filter(Rating ==  min(Rating, na.rm = T))
...
ggplot(rating_per_season) + geom_line(mapping = aes(x = Season, y = avg_rating))+ geom_point(mapping = aes(x = Season, y = avg_rating)) + geom_smooth(mapping = aes(x = Season, y = avg_rating))
